...
server <- function(input, output) {
  ...
  tag_train <- reactive({
    ...
    for (i in 1:nrow(x2)){
      ...
      #ifelse(length(keywords[str_detect(dat, keywords)]) == 0, '', paste(ifelse(keywords[str_detect(dat, keywords)][1] == 'NA',,keywords[str_detect(dat, keywords)][1]), ifelse(keywords[str_detect(dat, keywords)][2] == 'NA',,keywords[str_detect(dat, keywords)][2]), ifelse(keywords[str_detect(dat, keywords)][3] == 'NA',,keywords[str_detect(dat, keywords)][3]), keywords[str_detect(dat, keywords)][4], sep = ','))
    }
    return(x2)
  })
  ...
  output$sent_com <- renderPlot({
    ...
    #   
    #   x <- sent_train()
    #   m_service <- mean(subset(x, 'service' %in% strsplit(x, ',')), na.rm = TRUE)
    #   mx <- mean(x, na.rm = TRUE)
    #   
    #   y <- sent_test()
    #   y <- y$rating
